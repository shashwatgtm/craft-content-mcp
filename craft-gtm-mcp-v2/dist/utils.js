...
export function parseMetrics(metricsText) {
    ...
    if (arrMatch) {
        ...
        if (arrMatch[2]?.toLowerCase() === 'k')
            value *= 1000;
        if (arrMatch[2]?.toLowerCase() === 'm')
            value *= 1000000;
        metrics.arr = value;
    }
    // Churn rate
...
export function scoreMetric(value, benchmarks, higherIsBetter = true) {
    if (value === undefined) {
        return { score: 0, label: 'MISSING', analysis: 'Data not provided - unable to score' };
    }
    if (higherIsBetter) {
        if (value >= benchmarks.high)
            return { score: 9, label: 'EXCELLENT', analysis: `${value} exceeds benchmark of ${benchmarks.high}` };
        if (value >= benchmarks.medium)
            return { score: 7, label: 'GOOD', analysis: `${value} meets healthy benchmark of ${benchmarks.medium}` };
        if (value >= benchmarks.low)
            return { score: 5, label: 'DEVELOPING', analysis: `${value} is below target of ${benchmarks.medium}` };
        return { score: 3, label: 'CRITICAL', analysis: `${value} is significantly below minimum of ${benchmarks.low}` };
    }
    else {
        if (value <= benchmarks.low)
            return { score: 9, label: 'EXCELLENT', analysis: `${value}% is below target of ${benchmarks.low}%` };
        if (value <= benchmarks.medium)
            return { score: 7, label: 'GOOD', analysis: `${value}% is acceptable (benchmark: <${benchmarks.medium}%)` };
        if (value <= benchmarks.high)
            return { score: 5, label: 'DEVELOPING', analysis: `${value}% is elevated - needs attention` };
...
export function analyzeCRAFTDimensions(content) {
    const analysis = {
