...
function parseLaunchDate(dateInput) {
...
export function generateLaunchCommander(args) {
    ...
    const filterTasksByChannels = (tasks) => {
        return tasks.filter(task => {
            const taskLower = task.toLowerCase();
            if (channels.some(ch => taskLower.includes(ch.toLowerCase())))
                return true;
            if (['position', 'messaging', 'strategy', 'feedback', 'metric', 'monitor', 'plan', 'brief'].some(word => taskLower.includes(word)))
                return true;
            if (taskLower.includes('webinar') && !channels.some(ch => ch.includes('webinar')))
                return false;
            if (taskLower.includes('pr') && !channels.some(ch => ch.includes('pr')))
                return false;
            if (taskLower.includes('paid') && !channels.some(ch => ch.includes('paid')))
                return false;
            return true;
        });
    };
    // Adjust task assignment based on team size
    const getOwner = (taskType) => {
        const ownerMap = {
...
//# sourceMappingURL=launch-commander.js.map